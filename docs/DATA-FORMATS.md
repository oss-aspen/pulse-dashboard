...
- Fedora/Hummingbird sidecar dumps (`Package`, `upstream_repo`, `version`, optionally wrapped as `{ "sidecar": { ... } }`) can be converted with the stdlib Python script `modules/product-upstreams/scripts/from-sidecar.py`. Records that share `upstream_repo` become one upstream with multiple packages. Example: `python3 modules/product-upstreams/scripts/from-sidecar.py sidecars.json --merge`.
