...
describe('parseCsv', () => {
  ...
  it('returns empty records array when only header row is present', () => {
    const { headers, records } = parseCsv('Component;Product;QA Contact', { delimiter: ';' })
  ...
  it('accepts a comma delimiter for standard CSV', () => {
    const csv = 'Name,Role,Active\nAlice,Engineer,True\nBob,Manager,True'
    const { headers, records } = parseCsv(csv, { delimiter: ',' })
    expect(headers).toEqual(['Name', 'Role', 'Active'])
    expect(records).toHaveLength(2)
    expect(records[0]).toMatchObject({ Name: 'Alice', Role: 'Engineer', Active: 'True' })
  })

  it('correctly parses quoted fields with a comma delimiter', () => {
